...
const IssueBook = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/transactions/bookissue', {
        bookName,
        userName,
        issueDate,
      });
      ...
      } else if (response.data.message === 'Book issued successfully') {
        setMessage('Success: Book issued successfully.');
      } else {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-center">Issue Book</h2>
        {message && (
